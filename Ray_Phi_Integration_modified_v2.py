...
def prob_val(phi_min,phi_max,theta_min,theta_max,rho):
    ...
    [prob,error]=dblquad(lambda y, x: (numerix.sin(y) * rho([[numerix.sin(y)*numerix.cos(x)], [numerix.sin(y)*numerix.sin(x)], [numerix.cos(y)]], order=1)),phi_min,phi_max, lambda phi: theta_min, lambda phi: theta_max, epsabs=1e-15)#, epsrel=1e-13)
    return prob

def task_loop(worker_idx, total_tasks, phi_array, theta_array, rho):
    ...
    while global_idx < total_tasks:
        ...
        local_probVal = probVal(min_phi, max_phi, min_theta, max_theta, rho)
...
if __name__ == '__main__':
    ...
    for worker_idx in range(num_workers-1):
        task_result.append(task_loop(worker_idx, total_tasks, phi, theta, rho))
