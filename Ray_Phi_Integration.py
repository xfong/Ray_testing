...
def sphericalDatapoint(theta, phi):
  ...
  return sin_theta*getCellVariableDatapoint.remote([[sin_theta*cos_phi], [sin_theta*sin_phi], [cos_theta]])
#@ray.remote
def f(theta,phi):
	return sphericalDatapoint.remote(theta, phi)
...
for iphi in range(len(phi)-1):
	for ith in range(len(theta)-1):
		...
		#f = lambda y, x: sphericalDatapoint.remote(y, x)
		[prob,error] = (dblquad(lambda th, ph: f(th,ph), phi[iphi], phi[iphi+1], lambda phi:  theta[ith], lambda phi: theta[ith+1], epsabs=1e-15, epsrel=1e-13))
